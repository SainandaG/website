...
class TemporalAnalyzer:
    ...
    async def analyze_evolution(self, connection_id: str) -> Dict[str, Any]:
        ...
        if not table_timelines:
            ...
            genesis_date = datetime.now() - timedelta(days=365)
            for table in schema.tables:
        ...
        # Sort by birth date
        table_timelines.sort(key=lambda x: x['birth_date'] or datetime.now())
        
        # Normalize the timeline
        all_birth_dates = [t['birth_date'] for t in table_timelines if t['birth_date']]
        start_date = min(all_birth_dates, default=datetime.now() - timedelta(days=365))
        end_date = datetime.now().astimezone()
        
        # Ensure start_date is timezone-aware if end_date is
        if end_date.tzinfo and not start_date.tzinfo:
    ...
    async def _analyze_table_evolution(self, connection_id: str, table: Any) -> Optional[Dict[str, Any]]:
        ...
        if timestamp_col:
            ...
            if timestamp_col.lower() == 'year':
                ...
                try:
                    result = await db_connector.query(connection_id, sql)
                    if result and result[0]['birth_date']:
                        raw_date = result[0]['birth_date']
